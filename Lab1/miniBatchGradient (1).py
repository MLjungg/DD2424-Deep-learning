...
def ComputeGrads(X, Y, W, b, lamda, batch_size):
	P = evaluateClassifier(X, W, b)
	G = - (Y - P)
	dl_dw = (1 / batch_size) * np.dot(G, np.transpose(X)) + 2 * lamda * W
...
def compare_gradients(X_train, Y_train, W, b, GD_params, lamda):
	W_num, b_num = ComputeGradsNumSlow(X_train[:, 0:100], Y_train[:, 0:100], W, b, lamda, 1e-6)
	W_anal, b_anal = ComputeGrads(X_train[:, 0:100], Y_train[:, 0:100], W, b, lamda,
